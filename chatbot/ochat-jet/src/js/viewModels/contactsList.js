...
define(['ojs/ojcore', 'knockout', 'jquery',
    ...
    function contactsViewModel() {
        ...
        // load customers
        data.getContacts().then(function (response) {
        ...
        self.contacts = ko.computed(function () {
            ...
            if (self.nameSearch() && self.allContacts().length > 0) {
                var filteredContacts = [];

                var token = self.nameSearch().toLowerCase();

                self.allContacts().forEach(function (node) {
                    node.children.forEach(function (leaf) {
                        if (leaf.attr.firstName.toLowerCase().indexOf(token) === 0 || leaf.attr.lastName.toLowerCase().indexOf(token) === 0) {
                            filteredContacts.push(leaf);
                        }
                    });
                });
                return new oj.JsonTreeDataSource(filteredContacts);

            } else {
                return new oj.JsonTreeDataSource(self.allContacts());
            }
